...
class PongGame:
    ...
    def train_ai(self, genome1, genome2, config, draw=False):
        ...
        max_hits = 50 # To make sure that the AI doesn't enter into a recursive cycle and get stuck at the top or bottom
...
def run_neat(config):
    #p = neat.Checkpointer.restore_checkpoint('neat-checkpoint-85') # to restore previous checkpoints if the program crashes, the checkpoint will continue from the previous config.txt settings
...
if __name__ == '__main__':
    ...
    run_neat(config) # To train
    #test_best_network(config) # To play
